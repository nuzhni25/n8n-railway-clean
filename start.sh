...
# 🚨 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: ПРИНУДИТЕЛЬНОЕ ИСПОЛЬЗОВАНИЕ RAILWAY VOLUME БАЗЫ!
echo "🚨 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ - ОБЯЗАТЕЛЬНО используем Railway Volume базу..."

# РЕШЕНИЕ ПРОБЛЕМЫ: Railway Volume база должна использоваться БЕЗ копирования!
if [ -f "/app/database.sqlite" ]; then
    file_size=$(stat -c%s "/app/database.sqlite" 2>/dev/null || echo "0")
    echo "📊 Размер Railway Volume базы: $file_size байт"
    
    if [ "$file_size" -gt 50000000 ]; then
        echo "🎯 НАЙДЕНА БОЛЬШАЯ БАЗА В RAILWAY VOLUME!"
        echo "🔗 ИСПОЛЬЗУЕМ ЕЁ НАПРЯМУЮ БЕЗ СОЗДАНИЯ НОВОЙ!"
        
        # КЛЮЧЕВОЕ РЕШЕНИЕ: Используем Railway Volume базу НАПРЯМУЮ!
        DB_FILE="/app/database.sqlite"
        
        # Исправляем права доступа для node user
        chmod 664 "$DB_FILE" 2>/dev/null || true
        
        echo "✅ DB_FILE принудительно установлен: $DB_FILE"
        
        # КРИТИЧНО: Проверяем что база НЕ пустая
        table_count=$(sqlite3 "$DB_FILE" "SELECT COUNT(*) FROM sqlite_master WHERE type='table';" 2>/dev/null || echo "0")
        echo "📊 Количество таблиц в Railway Volume базе: $table_count"
        
        if [ "$table_count" -gt 5 ]; then
            echo "🎉 БАЗА СОДЕРЖИТ ДАННЫЕ! Это ПРАВИЛЬНАЯ база!"
        else
            echo "⚠️ База пустая или повреждена ($table_count таблиц)"
        fi
    else
    ...
    echo "❌ Railway Volume база не найдена"
    DB_FILE="/home/node/data/database.sqlite"
fi

# 🔧 УСТАНАВЛИВАЕМ ВСЕ ПЕРЕМЕННЫЕ ОКРУЖЕНИЯ для n8n
...
# 🚨 КРИТИЧНО: Дополнительные переменные для точного указания базы
export N8N_DB_SQLITE_DATABASE="$DB_FILE"
export SQLITE_DATABASE="$DB_FILE"
